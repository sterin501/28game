...
def whoGotit(thisPlay,rules,tt,FristKalli):
...
def setThurpu(FristKalli,rules):
    print ('Set Trump Card please')
    c=0
    while len (rules.skipped) < 4:

           if not FristKalli[c].name in rules.skipped:
              settingTrump(FristKalli[c],rules)
           c=c+1
           if c == 4:
               c = 0
...
if __name__ == "__main__":
    P1=game.Player("p1","Team0",True)
    P2=game.Player("p2","Team1",True)
    P3=game.Player("p3","Team0",True)
    P4=game.Player("p4","Team1",False)
    ...
    while True:
        ...
        doTheDeal(P1,P2,P3,P4)
        P4.showHand()
        P4.showHandinUTF()
        FristKalli=tt.getFirstKalli()
        print ("Frist player (Kallikaran )  is  "+ FristKalli[0].name)
        tt.setFirstKalli()
        rules=RuleBook.Points()

        setThurpu(FristKalli,rules)
        ...
        for kk in range(1,9):
           ...
           thisPlay=[]
           for cc in range(0,4):
                thisPlay.append(makeMove(FristKalli[cc],False,"",thisPlay))
           print (thisPlay)
           FristKalli=whoGotit(thisPlay,rules,tt,FristKalli)
